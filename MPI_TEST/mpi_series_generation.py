...
def sphereclump_series_simulation_specific(mpi):
    filename = "sphereclump"
    ...
    simulation.extra =  str(mpi[0]) + "_" + str(mpi[1]) + "_" + str(mpi[2])
...
def make_and_gen(remake_base_particle_shapes, make_vtk_files):
    ...
    mpi_settings = [[3,2,1], [3,3,2], [4,4,2], [5,5,2], [6,6,3]]
    for mpi in mpi_settings:
        simulation_sphereclump = sphereclump_series_simulation_specific(mpi)
        simulation_sphereclump.generate_liggghts_files(mpi, random_orientation=False)
        if make_vtk_files:
            simulation_sphereclump.particletemplate.particle.legacy_vtk_printout()


def sphereclump_series_validate_liggghts():
    simulation = sphereclump_series_simulation_specific()
...
########################
#sphereclump_series_validate_liggghts()
