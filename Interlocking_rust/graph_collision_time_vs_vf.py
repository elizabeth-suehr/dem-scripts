...
for i in range(len(directory_1)):
    ...
    for j in range(len(directory_2)):
        ...
        filename = (
            "collision_check/"+
             directory_1[i] + directory_2[j] + "collision.data"
        )
        list_of_particles = []
        with open(filename) as f:
            ...
            for line in f:
                ...
                if stringvalues[0] == "file":
                    ...
                    if not list_of_particles.contains(stringvalues[0]):
                        count += 1
                        list_of_particles.append(stringvalues[0])
                    if not list_of_particles.contains(stringvalues[1]):
                        count += 1
                        list_of_particles.append(stringvalues[1])
...
for i in range(len(directory_1)):
    for j in range(len(directory_2)):
        ...
        print(len(collision_counts[i][j]))
...
plt.figure(1)
marks = ["o", "v", "x", "s", "*", "D", "+"]
labelnames = ["Curl 0", "Curl 1", "Curl 2", "Curl 3", "Curl 4", "Curl 5"]
curls = [object] * 6
for i in range(len(directory_1)):
    (curls[i],) = plt.plot(
        volume_fraction, collision_averages[i][:], "--", marker=marks[i]
    )
...
plt.legend(
    ...
    labelnames,
    handler_map={
        curls[0]: curl_image_0,
        curls[1]: curl_image_1,
        curls[2]: curl_image_2,
        curls[3]: curl_image_3,
...
for i in range(len(directory_1)):
    (curls[i],) = plt.plot(
        volume_fraction, collision_percentages[i][:], "--", marker=marks[i]
    )

plt.xlabel("Solid volume fraction (ν)")
plt.ylabel("Percentage Interlocked")
plt.legend(
    curls,
    labelnames,
    handler_map={
...
plt.savefig("percentage_interlocked_vs_vf.pdf", bbox_inches="tight")
# for i in range(len(directory_1)):
